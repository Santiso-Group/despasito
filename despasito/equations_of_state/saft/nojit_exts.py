    ...
    if np.size(np.shape(l_kl)) == 2:
        ...
        for k in range(nbeads):
            for l in range(nbeads):
                cikl = np.inner(
                    ...
                    np.transpose(
                        np.array(
                            [1.0, l_kl[k, l] ** -1, l_kl[k, l] ** -2, l_kl[k, l] ** -3]
                        )
                    ),
                )
        ...
        for k in range(nbeads):
            cikl = np.inner(
                constants.ckl_coef,
                np.transpose(
                    np.array([1.0, l_kl[k] ** -1, l_kl[k] ** -2, l_kl[k] ** -3])
                ),
            )
    ...
    if np.size(np.shape(l_kl)) == 2:
        ...
        for k in range(nbeads):
            for l in range(nbeads):
                ...
                etakl[:, k, l] = np.einsum("ij,j", zetax_pow, cikl)
                rhos_detakl_drhos[:, k, l] = np.einsum(
                    "ij,j", zetax_pow, cikl * np.array([1.0, 2.0, 3.0, 4.0])
                )
        da1s_drhos = np.einsum(
            "ijk,jk->ijk",
            (1.0 - (etakl / 2.0)) / ((1.0 - etakl) ** 3)
            + (5.0 - 2.0 * etakl) / (2.0 * (1.0 - etakl) ** 4) * rhos_detakl_drhos,
...
# @profile
def calc_Xika(
    indices, rho, xi, nui, nk, Fklab, Kklab, Iij, maxiter=500, tol=1e-12, damp=0.1
):
    r""" 
