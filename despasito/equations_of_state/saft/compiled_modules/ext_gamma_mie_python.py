...
def calc_a1ii(rho, Cmol2seg, dkl, l_akl, l_rkl, x0kl, epsilonkl, zetax):
    ...
    output = (Cii * (((x0kl**l_akl) / constants.molecule_per_nm3 * (a1s_a + Bii_a)) - ((x0kl**l_rkl) / constants.molecule_per_nm3 * (a1s_r + Bii_r))))
    ...
    for p in range(11):
        for q in range(11 - p):
            ...
            if p == 0:
                ...
                rho2 = rho**2
                Iij += np.einsum("i,jk->ijk", cij[p, q] * ((sigmax3**p * rho2)), ((T / epsilonij)**q))
            elif p == 3:
                rho3 = rho2*rho
                Iij += np.einsum("i,jk->ijk", cij[p, q] * ((sigmax3**p * rho3)), ((T / epsilonij)**q))
            elif p == 4:
                rho4 = rho2**2
                Iij += np.einsum("i,jk->ijk", cij[p, q] * ((sigmax3**p * rho4)), ((T / epsilonij)**q))
            elif p == 5:
                rho5 = rho2*rho3
                Iij += np.einsum("i,jk->ijk", cij[p, q] * ((sigmax3**p * rho5)), ((T / epsilonij)**q))
            elif p == 6:
                Iij += np.einsum("i,jk->ijk", cij[p, q] * ((sigmax3**p * rho*rho5)), ((T / epsilonij)**q))
            elif p == 7:
                Iij += np.einsum("i,jk->ijk", cij[p, q] * ((sigmax3**p * rho2*rho5)), ((T / epsilonij)**q))
            elif p == 8:
                Iij += np.einsum("i,jk->ijk", cij[p, q] * ((sigmax3**p * rho3*rho5)), ((T / epsilonij)**q))
            elif p == 9:
                Iij += np.einsum("i,jk->ijk", cij[p, q] * ((sigmax3**p * rho4*rho5)), ((T / epsilonij)**q))
            elif p == 10:
                Iij += np.einsum("i,jk->ijk", cij[p, q] * ((sigmax3**p * rho5*rho5)), ((T / epsilonij)**q))
