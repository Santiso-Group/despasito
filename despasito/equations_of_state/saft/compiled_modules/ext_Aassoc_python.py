...
def calc_Xika(indices, rho, xi, molecular_composition, nk, Fklab, Kklab, gr_assoc, maxiter=500, tol=1e-12, damp=.1):
    r""" 
...
#    Xika_final = np.ones((nrho,ncomp, nbeads, nsitesmax))
    ...
    Xika_elements_old = .5*np.ones(len(indices))
    for r in range(nrho):
        for knd in range(maxiter):
            ...
            for ind in range(l_ind):
                ...
                for jnd in range(l_ind):
                    ...
                    Xika_elements_new[ind] += constants.molecule_per_nm3 * rho[r] * xi[j] * molecular_composition[j,l] * nk[l,b] * Xika_elements_old[jnd] * delta
