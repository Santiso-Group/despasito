...
bead_library = {'H2O353': {'epsilon': 479.56, 'lambdaa': 6.0, 'lambdar': 8.0, 'sigma': 3.0029e-1, 'Sk': 1.0, 'Vks': 1, 'mass': 0.018015},
                'CO2': {'epsilon': 353.55, 'lambdaa': 6.66, 'lambdar': 23.0, 'sigma': 3.741e-1, 'Sk': 1.0, 'Vks': 1, 'mass': 0.04401},
                "CH3CH2CH2-": {"Sk": 1.0, "Vks": 1.0, "epsilon": 342.0, "lambdaa": 6.0, "lambdar": 15.0, "mass": 0.04309, "sigma": 0.45089},
               }
cross_library = {'CO2': {'H2O353': {'epsilon': 432.69}}, "H2O353":{"CH3CH2CH2-":{"epsilon": 315.7604284}}}

beads_co2_h2o = ['CO2', 'H2O353']
molecular_composition_co2_h2o = np.array([[1., 0.],[0., 1.]])
Eos_co2_h2o = despasito.equations_of_state.initiate_eos(eos="saft.gamma_mie",beads=beads_co2_h2o,molecular_composition=molecular_composition_co2_h2o,bead_library=bead_library,cross_library=cross_library, numba=True)

beads_co2_h2o = ['H2O353', 'CH3CH2CH2-']
molecular_composition_co2_h2o = np.array([[1., 0.],[0., 2.]])
Eos_h2o_hexane = despasito.equations_of_state.initiate_eos(eos="saft.gamma_mie",beads=beads_co2_h2o,molecular_composition=molecular_composition_co2_h2o,bead_library=bead_library,cross_library=cross_library)
...
def test_activity_coefficient(Eos=Eos_h2o_hexane, Tlist=Tlist, xilist=xilist, yilist=yilist, Plist=Plist):
    ...
    output = thermo.thermo(Eos,calculation_type="activity_coefficient",**{"Tlist":Tlist,"Plist":Plist,"yilist":yilist,"xilist":xilist})

    print(output["gamma"])
    assert output["gamma"][0]==pytest.approx(np.array([7.23733364e+04, 6.30243983e-01]),abs=1e-2) 
