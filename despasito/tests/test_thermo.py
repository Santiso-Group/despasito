...
def test_phase_xiT(eos=eos_co2_h2o,Tlist=Tlist,xilist=xilist):
    ...
    assert output["P"][0]==pytest.approx(4555722.774185303,abs=1e+1) and output["yi"][0]==pytest.approx([0.99539961, 0.00460039],abs=1e-4)
...
def test_phase_yiT(eos=eos_co2_h2o,Tlist=Tlist,yilist=yilist):
    ...
    assert output["P"][0]==pytest.approx(4300135.363671318,abs=1e+1) and output["xi"][0]==pytest.approx([0.9617648, 0.0382352],abs=1e-4)
...
def test_liquid_properties(eos=eos_co2_h2o,Tlist=Tlist,xilist=xilist,Plist=Plist):
    ...
    assert output["rhol"][0]==pytest.approx(17098.9876440635,abs=1e-1) and output["phil"][0]==pytest.approx(np.array([84.79316501,  0.21474978]),abs=1e-1)
