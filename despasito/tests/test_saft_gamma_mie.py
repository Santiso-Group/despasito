...
nui_co2_ben = np.array([[1.0, 0.0], [0.0, 1.0]])
beadlibrary_co2_ben = {
    "CO2": {
        "epsilon": 361.69,
        "l_a": 6.66,
...
def test_saft_gamma_mie_class_noassoc(
    beads=beads_co2_ben, nui=nui_co2_ben, beadlibrary=beadlibrary_co2_ben
):
    #   """Test ability to create EOS object without association sites"""
    eos_class = despasito.equations_of_state.eos(
        eos="saft.gamma_mie", beads=beads, nui=nui, beadlibrary=beadlibrary
    )
...
def test_saft_gamma_mie_class_assoc_mu(
    ...
    assert phi == pytest.approx(np.array([4.35827475, 0.02554506]), abs=1e-4)
