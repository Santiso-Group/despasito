...
bead_library = {'acetone': {'Tc': 508.1, 'Pc': 4690000.0, 'omega': 0.304}, 'chloroform': {'Tc': 536.4, 'Pc': 5471550.0, 'omega': 0.221902}}
cross_library = {"acetone": {"chloroform": {"kij": -0.0605}}}

Eos = despasito.equations_of_state.initiate_eos(eos="cubic.peng_robinson", xi=xi, beads=beads,molecular_composition=molecular_composition,bead_library=bead_library,cross_library=cross_library)
T = 332.15 
P = np.array([101330.0])
...
def test_PR_coefficients(xi=xi,beads=beads,molecular_composition=molecular_composition,bead_library=bead_library):  #   """Test ability to create EOS object without association sites"""
    Eos_class = despasito.equations_of_state.initiate_eos(eos="cubic.peng_robinson",xi=xi,beads=beads,molecular_composition=molecular_composition,bead_library=bead_library)
    tmp = [Eos_class.bead_library[beads[0]]['kappa'], Eos_class.bead_library[beads[1]]['kappa']]
