...
bead_library = {'H2O': {'epsilon': 250.0, 
                        'lambda': 1.7889, 
                        'sigma': 3.0342e-1, 
                        'Sk': 1.0, 
                        'Vks': 1.0, 
...
Kklab = np.array([[[[[[   0., 0.00106673], \
                    [0.00106673, 0.   ]]]]]])

Eos = despasito.equations_of_state.initiate_eos(eos="saft.gamma_sw",beads=bead, molecular_composition=molecular_composition, bead_library=copy.deepcopy(bead_library))
T = 580.0
density = np.array([40004.84463113798]) 
P = np.array([9447510.360679299])

def test_saft_gamma_sw_class_bonding_volume(T=T, beads=bead, molecular_composition=molecular_composition, bead_library=bead_library, Kklab=Kklab):
#   """Test ability to create EOS object with association sites"""
    Eos_class = despasito.equations_of_state.initiate_eos(eos="saft.gamma_sw",beads=beads,molecular_composition=molecular_composition,bead_library=copy.deepcopy(bead_library))
...
#   """Test ability to predict P with association sites"""
    ...
    assert P == pytest.approx(9447510.360679299,abs=1e+3)
