...
class Data(ExpDataTemplate):
    ...
    def __init__(self, data_dict):
        ...
        logger.info(
            "Data type 'sat_props' initiated with calctype, {}, and data types: {}.\nWeight data by: {}".format(
    ...
    def objective(self, eos):
        ...
        if "Psat" in self._thermodict:
            obj_value[0] = np.sum(
                (
                    (
                        (phase_list[0] - self._thermodict["Psat"])
                        / self._thermodict["Psat"]
                    )
                    ** 2
                )
                * self.weights["Psat"]
        ...
        if "rhov" in self._thermodict:
            obj_value[2] = np.sum(
                (
                    (
                        (phase_list[2] - self._thermodict["rhov"])
                        / self._thermodict["rhov"]
                    )
                    ** 2
                )
                * self.weights["rhov"]
