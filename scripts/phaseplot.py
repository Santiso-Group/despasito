    ...
    for j,bd in enumerate(specs):
        ...
        for i,t in enumerate(T):
            for k,sim in enumerate(sim_types):
                ...
                fname = "{}{}_{}_{}/out_{}_{}_{}.txt".format(basepath,system,t,bd,system,t,sim)
                ...
                plot_sim_Pxy_data(sim_data, debug_data, label, fig_num=fig_num, style=style[k], color=col[i], debug_plots=debug_plots)
    
        art = [plt.legend(loc='center left', bbox_to_anchor=(1, 0.5))]
        plt.title(title)
        plt.xlabel('$x_1$, $y_1$')
        plt.ylabel('P (MPa)')
        plt.savefig('{}_{}.png'.format(system,bd),dpi=400,additional_artists=art,bbox_inches="tight")
        if plot_show:
    ...
    for i,line in enumerate(contents):
        ...
        if T != float(linearray[0]) or flag == 1:
            ...
            if len(exp_data) > 0:
                ...
                exp_data[-1][0] = exp_data[-1][0]*1e+6
            exp_data.append([])
...
def plot_exp_Pxy_data(Nfigures, calc_temps, data, style="o><^", c_array='rbgcmk'):
    ...
    for i,temp in enumerate(calc_temps):
        ...
        print(temp,data[0])
        jnd = np.where(np.abs(temp-data[0])<1.)[0]
        ...
        for j in range(len(jnd)):
            ...
            for f in range(Nfigures):
                fig = f+1
                plt.figure(fig)
                plt.plot(data[2][jnd[j]][1],data[2][jnd[j]][0],style[j]+c_array[i],label=tmplabel)
...
def plot_sim_Pxy_data(sim_data, debug_data, label, fig_num=1, style="", color='k', linewidth=2, debug_plots=False, debug_path="",debug_cut=1e-5):
    ...
    if debug_plots:
        ...
        for i in range(len(sim_data)):
            ...
            if log_obj[i] > debug_cut: jnd = 1
            else: jnd = 0
            # Check liquid flags
            if debug_data[0][i] not in [1, 2]: ind1 = 1
            else: ind1 = 0
            # Check vapor flags
            if debug_data[1][i] not in [0, 2, 4]: ind2 = 1
            else: ind2 = 0
            # Plot new point
            if ind1 == 1:
                plt.plot(sim_data[1][i],sim_data[0][i],marker=markers[ind1],color="k")
            if (ind2 == 1 or jnd == 1):
                plt.plot(sim_data[2][i],sim_data[0][i],marker=markers[ind2],color=color[jnd])

def ADD_Pxy_data(exp_data, sim_data, filename="ADD.csv", column_string="", column_header=""):
    ...
    for i,data in enumerate(sim_data):
        ...
        AAD_tmp = []
        for j in range(l_c):
            AAD_tmp.append(str(np.nanmean(np.abs((data[j]-exp_data[j])/exp_data[j]))*100))
